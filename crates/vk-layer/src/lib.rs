...
/// Pack composites actually recorded over intercepted frames — the positive
/// cross-cdylib confirmation that `PackCompositor::record_over` ran, not
/// just that trigger passes were classified.
#[unsafe(no_mangle)]
pub extern "system" fn ferridian_layer_composite_count() -> u64 {
    hooks::composite_count()
}

/// JNI entry point for `io.ferridian.shim.contract.NativeBridge#publishCamera`
/// (generated by `tools/shim-codegen` from the same std140 field table).
/// `_env`/`_class` are opaque JNI handles this function never dereferences,
/// so it needs no `unsafe` beyond the ABI it advertises.
#[unsafe(no_mangle)]
pub extern "system" fn Java_io_ferridian_shim_contract_NativeBridge_publishCamera(
    _env: *mut c_void,
    _class: *mut c_void,
    sun_direction_x: f32,
    sun_direction_y: f32,
    ...
    camera_transport::publish(CameraUniforms {
        sun_direction: [sun_direction_x, sun_direction_y, sun_direction_z],
        time_seconds,
        near_plane,
        far_plane,
    });
...
/// Erase the boilerplate of one JNI getter mirroring a `CameraUniforms`
/// field: same opaque-handle non-safety argument as `publishCamera` above.
macro_rules! camera_getter {
    ($jni_name:ident, $field:expr) => {
        #[unsafe(no_mangle)]
        pub extern "system" fn $jni_name(_env: *mut c_void, _class: *mut c_void) -> f32 {
            $field(camera_transport::current_camera())
        }
    };
}

camera_getter!(
    Java_io_ferridian_shim_contract_NativeBridge_currentSunDirectionX,
    |c: CameraUniforms| c.sun_direction[0]
);
camera_getter!(
    Java_io_ferridian_shim_contract_NativeBridge_currentSunDirectionY,
    |c: CameraUniforms| c.sun_direction[1]
);
camera_getter!(
    Java_io_ferridian_shim_contract_NativeBridge_currentSunDirectionZ,
    |c: CameraUniforms| c.sun_direction[2]
);
camera_getter!(
    Java_io_ferridian_shim_contract_NativeBridge_currentTimeSeconds,
    |c: CameraUniforms| c.time_seconds
);
camera_getter!(
    Java_io_ferridian_shim_contract_NativeBridge_currentNearPlane,
    |c: CameraUniforms| c.near_plane
);
camera_getter!(
    Java_io_ferridian_shim_contract_NativeBridge_currentFarPlane,
    |c: CameraUniforms| c.far_plane
);
...
unsafe fn end_render_pass_common(command_buffer: vk::CommandBuffer, forward: impl FnOnce()) {
    ...
    dispatch::with_device(key, |state| {
        ...
        forward();
        // Which image every labeled pass renders into — the map that pins
        // where the presented frame actually comes from on a live game.
        if tracing::enabled!(tracing::Level::TRACE)
            && let Some(pass) = pass.as_ref()
        {
            let resolved = pass.resolve(&state.taps.lock().expect("tap registry poisoned"));
            tracing::trace!(
                kind = pass.kind.as_str(),
                label = pass.label.as_deref().unwrap_or("<none>"),
                dynamic = matches!(pass.geometry, PassGeometry::Dynamic { .. }),
                color = resolved.map(|frame| frame.color.image.as_raw()),
                layout = resolved.map(|frame| frame.color.final_layout.as_raw()),
                depth = resolved
                    .and_then(|frame| frame.depth)
                    .map(|depth| depth.image.as_raw()),
                width = pass.render_area.extent.width,
                height = pass.render_area.extent.height,
                "pass ended"
            );
        }
        ...
        if let Some(pass) = pass.as_ref().filter(|pass| composite_trigger(pass.kind)) {
            ...
            if let (Some(compositor), Some(frame)) = (compositor.as_mut(), resolved) {
                // Whatever the shim published most recently (the placeholder
                // if nothing has, e.g. no Fabric shim or no real game yet).
                compositor.update_camera(&camera_transport::current_camera());
                // SAFETY: we are on the app's recording thread, immediately
                // after its render pass ended in this command buffer.
                if unsafe { compositor.record_over(command_buffer, &frame) } {
                    hooks::composite_recorded(pass.kind);
                }
            } else if pack_active {
                hooks::composite_unresolved(pass.kind);
            }
        }
    });
}
